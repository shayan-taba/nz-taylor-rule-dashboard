...
function interpretation(
  ...
  const biasStr =
    meanDev > 0.3  ? "hawkish bias" :
    meanDev < -0.3 ? "dovish bias"  :
    "near-neutral";
...
export function RegimeComparisonTable() {
  ...
  const rows: RegimeRow[] = useMemo(() => {
    ...
    return governors.map((regime) => {
      ...
      const regimeEnd   = regime.endDate?.split("T")[0]
      ...
      const getInflation = (raw: typeof regimeRaw[0]): number | null => {
        ...
        if (inflationMeasure === "inflationTrimmed")  return raw.inflationTrimmed;
        if (inflationMeasure === "inflationSectoral") return raw.inflationSectoral;
      ...
      const olsValid = regimeRaw
        ...
        .filter(Boolean) as { lhs: number; infGap: number; outputGap: number }[];

      const n = olsValid.length;
      let alpha = null, beta = null, r2 = null, rmse = null;
  ...
  return (
    <div style={{
              ...
              return (
                ...
                >
                  ...
                  <td style={{ ...cellStyle, color: "var(--text-2)", whiteSpace: "nowrap" }}>
                    {toQLabel(regime.startDate)} — {regime.endDate ? toQLabel(regime.endDate) : "Present"}
                  </td>
      ...
      <div style={{
        ...
        <div>
          <span style={{ color: "var(--text-2)" }}>α, β, R², RMSE</span>
          {" "}are estimated by OLS over each governor's{" "}
          <span style={{ color: "var(--text-2)" }}>full term</span>
          {" "}(fixed regime window). They do not change when you adjust the date
          range selector or switch regime tabs — they always reflect the
          complete historical record for that governor.
          The regression is (OCR − r*) = α·(π − π*) + β·gap with no intercept,
          consistent with the nominal r* formulation used throughout.
          Inflation measure:{" "}
        ...
        <div>
          <span style={{ color: "var(--accent-2)" }}>† Mean Dev</span>
          {" "}is the only column that responds to the date range selector.
          It shows the average deviation within the intersection of the
          governor's term and your currently selected window — useful for
          isolating a sub-period (e.g. Orr's response during COVID only).
          When the full history is selected it equals the governor's full-term average.
        </div>
      </div>
    </div>
  );
