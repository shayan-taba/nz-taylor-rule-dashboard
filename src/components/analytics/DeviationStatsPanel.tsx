...
function StatRow({
  ...
  return (
    <div style={{
      ...
      <span style={{ fontSize: "13px", color: color ?? "var(--text)", fontWeight: 400 }}>
        {value}
      </span>
    </div>
  );
...
export function DeviationStatsPanel() {
  ...
  return (
    <div style={{
      ...
      {!stats ? (
        <div style={{ color: "var(--text-3)", fontSize: "11px" }}>No data in selected window</div>
      ) : (
        <>
          <StatRow label="OBSERVATIONS"     value={`${stats.n} quarters`} />
          <StatRow label="MEAN DEVIATION"   value={fmt(stats.mean, 2, true)}
          ...
          <div style={{ marginTop: 20 }}>
            <div style={{ fontSize: "9px", color: "var(--text-3)", letterSpacing: "0.1em", marginBottom: 6 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: "9px", color: "var(--text-3)", marginTop: 4 }}>
