    ...
    >
      ...
      >
        <div>
          ...
          <div
            style={{ fontSize: "9px", color: "var(--text-3)", marginTop: 2 }}
          >
            {xLabel} vs {yLabel}
          </div>
        </div>
      ...
      {!dots.length ? (
        ...
        <ResponsiveContainer width="100%" height={220}>
          <ComposedChart margin={{ top: 4, right: 4, bottom: 16, left: -10 }}>
            ...
            />
            {/* Scatter dots */}
            <Scatter data={dots} fill="var(--accent)" fillOpacity={0.5} r={3} />
            {/* OLS trend line */}
            <Line
              data={trendLine}
              dataKey="y"
...
export function ScatterPlots() {
  ...
  const { inflPairs, uratePairs, nzdPairs } = useMemo(() => {
    ...
    for (const row of data) {
      ...
      if (row.inflation !== null)
        inflPairs.push([row.inflation, row.deviation]);
