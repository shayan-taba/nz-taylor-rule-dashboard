...
export async function runFullPipeline(): Promise<void> {
  ...
  >(
    inflationRows.map((r) => [
      r.date.trim(),
      {
        ...
        sectoral: r.inflation_sectoral
          ? parseFloat(r.inflation_sectoral)
          : null,
  ...
  console.log(
    "[pipeline] neutral_rate keys (first 3):",
    [...neutralMap.keys()].slice(0, 3),
  );
  console.log(
    "[pipeline] inflation_manual keys (first 3):",
    [...inflationMap.keys()].slice(0, 3),
  );

  // 2. Parse MPS
  console.log("[pipeline] Parsing MPS XLSX...");
  const mpsPath = path.join(
    process.cwd(),
    "src/data",
    "latests/mps-latest.xlsx",
  );
  const { rows: mpsRows, vintage } = parseMps(mpsPath);
  console.log(
    `[pipeline] MPS: ${mpsRows.length} actual rows (vintage: ${vintage})`,
  );

  // Debug: log first few MPS date keys
  console.log(
    "[pipeline] MPS date keys (first 3):",
    mpsRows.slice(0, 3).map((r) => toDateKey(r.date)),
  );

  // 3. Parse HB1
  console.log("[pipeline] Parsing HB1 XLSX...");
  const hb1Path = path.join(
    process.cwd(),
    "src/data",
    "latests/hb1-latest.xlsx",
  );
