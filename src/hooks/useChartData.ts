...
function getInflationField(raw: QuarterlyData, measure: string): number | null {
  ...
  if (measure === "inflationTrimmed") {
    return raw.inflationTrimmed;
  }

  if (measure === "inflationSectoral") {
...
function buildCardInfo(
  ...
  const lag = Math.max(
    ...
    quarterCount(currentDate) - quarterCount(valueDate),
  );
...
export function useLagInfo(): LagInfo {
  ...
  return useMemo(() => {
    ...
    const inflValue = latestInflRow
      ? getInflationField(latestInflRow, inflationMeasure)
      : null;
    ...
    return rawSeries
      ...
      .map((raw): MergedRow => {
        ...
        return {
           ...
           * Recharts time scale
           */
          timestamp: new Date(dateKeyValue).getTime(),
